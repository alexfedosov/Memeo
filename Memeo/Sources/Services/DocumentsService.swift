...
class DocumentsService {
    ...
    func create(fromMedia url: URL, copyToDocumentsDir: Bool = true) async throws -> Document {
        logger.info("Creating document from media: \(url.lastPathComponent), copyToDocumentsDir: \(copyToDocumentsDir)", category: .fileOperations)
        var assetUrl = url
        ...
        if copyToDocumentsDir {
            guard
                ...
                logger.error("Failed to resolve document directory path", category: .fileOperations)
                throw DocumentServiceError.error("failed to resolve document directory path")
            }

            logger.info("Copying media file to documents directory: \(importURL.lastPathComponent)", category: .fileOperations)
        ...
        guard let videoTrack = try await asset.loadTracks(withMediaType: .video).first else {
            logger.error("No video track found in asset: \(assetUrl.lastPathComponent)", category: .fileOperations)
        ...
        logger.info("Successfully created document from media - duration: \(duration.seconds)s, keyframes: \(numberOfKeyframes), frameSize: \(frameSize)", category: .fileOperations)
    ...
    func create(fromImage image: UIImage) async throws -> Document {
        logger.info("Creating document from image - size: \(image.size)", category: .fileOperations)
        guard let url = VideoExporter().export(image: image) else {
            logger.error("Failed to export image to video", category: .fileOperations)
            throw DocumentServiceError.unexpectedError
        }
