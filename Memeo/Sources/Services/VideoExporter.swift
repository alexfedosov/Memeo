...
class VideoExporter {
    ...
    func exportGif(url: URL, trim: Bool = true) -> URL? {
        logger.info("Starting GIF export from video: \(url.lastPathComponent), trim: \(trim)", category: .videoProcessing)
        ...
        logger.info("Executing FFmpeg command for GIF export: \(command)", category: .videoProcessing)
        let _ = FFmpegKit.execute(command)
        let success = FileManager().fileExists(atPath: outfileURL.path)
        if success {
            logger.info("Successfully exported GIF to: \(outfileURL.lastPathComponent)", category: .videoProcessing)
        } else {
            logger.error("GIF export failed, output file not found: \(outfileURL.path)", category: .videoProcessing)
        }
        return success ? outfileURL : nil
    }

    func export(image: UIImage) -> URL? {
        logger.info("Starting image export - size: \(image.size)", category: .videoProcessing)
