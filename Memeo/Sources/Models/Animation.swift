...
extension Animation where T == CGPoint {
  func makeCAAnimation(numberOfKeyframes: Int, duration: CFTimeInterval, frameSize: CGSize) -> CAKeyframeAnimation {
    ...
    animation.values = keys
      ...
      .map {
        CGPoint(x: frameSize.width * $0.x, y: frameSize.height * $0.y)
      }
...
extension Animation where T == Bool {
  func toAnimatedValue(value: Bool, isPlaying: Bool) -> NSNumber {
    min((value ? 1 : 0) + (isPlaying ? 0 : 0.3), 1) as NSNumber
  }

  func makeCAAnimation(numberOfKeyframes: Int, duration: CFTimeInterval, isPlaying: Bool) -> CAKeyframeAnimation {
    let animation = CAKeyframeAnimation(keyPath: key)
    ...
    let animationData = keys
      .map {
      (time: Double($0 + (keyframes[$0] == true ? 0 : 0)) / Double(numberOfKeyframes), value: keyframes[$0])
    }
