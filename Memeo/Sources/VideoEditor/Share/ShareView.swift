...
struct ShareView: View {
    ...
    var body: some View {
        ...
        .background(
            Color.black.opacity(viewModel.isShown.wrappedValue ? 0.8 : 0).onTapGesture {
    ...
    func buttons() -> some View {
        HStack(alignment: .lastTextBaseline) {
            Button(
                action: { viewModel.shareToInstagram() },
                label: {
                    VStack {
                        Image("instagram")
                            .resizable()
            ...
            if let _ = viewModel.gifURL {
                Spacer()
                Button(
                    action: { viewModel.copyGifToPasteboard() },
                    label: {
                        VStack {
            ...
            Spacer()
            Button(
                action: { viewModel.saveToPhotoLibrary() },
                label: {
                    VStack {
            ...
            Spacer()
            Button(
                action: { viewModel.showMoreSharingOptions() },
                label: {
                    VStack {
