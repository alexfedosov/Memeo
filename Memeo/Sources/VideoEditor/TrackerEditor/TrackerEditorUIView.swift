...
class TrackersEditorUIView: UIView {
    ...
    func forceUpdateTrackers() {
        updateTrackers(
            newTrackers: trackerLayers.map {
                $0.tracker
            }, numberOfKeyframes: numberOfKeyframes, isPlaying: isPlaying, duration: duration,
            selectedTrackerIndex: selectedTrackerIndex, forceUpdate: true)
    }
    ...
    @objc func handlePanGesture(sender: UIPanGestureRecognizer) {
        ...
        case .began:
            ...
            touchLocationInMovingTracker =
                tracker.convert(location, from: layer)
                - CGPoint(x: tracker.bounds.width / 2, y: tracker.bounds.height / 2)
        case .changed:
            ...
            if let tracker = getTrackerModel(for: movingTracker) {
                let position = CGPoint(
                    x: movingTracker.position.x / bounds.size.width,
    ...
    func updateTrackers(
        newTrackers: [Tracker], numberOfKeyframes: Int, isPlaying: Bool, duration: CFTimeInterval,
        selectedTrackerIndex: Int?, forceUpdate: Bool = false
    ) {
        ...
        for change in diff {
            ...
            case .insert(let offset, let element, associatedWith: _):
                let trackerLayer = TrackerLayerRepresentable(
                    tracker: element, isSelected: offset == selectedTrackerIndex)
        ...
        for (index, newTracker) in newTrackers.enumerated() {
            ...
            if newTracker.position != trackerLayers[index].tracker.position || needUpdate {
                let positionAnimation = newTracker.position.makeCAAnimation(
                    numberOfKeyframes: numberOfKeyframes, duration: duration, frameSize: bounds.size)
            ...
            if newTracker.fade != trackerLayers[index].tracker.fade || needUpdate {
                let opacityAnimation = newTracker.fade.makeCAAnimation(
                    numberOfKeyframes: numberOfKeyframes, duration: duration, isPlaying: isPlaying)
