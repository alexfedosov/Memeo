        ...
        VStack {
            VStack {
                ...
                DialogGradientButton(
                    text: isLastSlide ? String(localized: "Close") : String(localized: "One more tip"),
                    action: {
                        if isLastSlide {
                            withAnimation {
                                isPresented = false
...
extension View {
    public func presentHelpView(isPresented: Binding<Bool>) -> some View {
        self.modifier(
            FullscreenModifier(
                presenting: HelpView(isPresented: isPresented), canCancelByBackgroundTap: true, isPresented: isPresented
            )
        )
